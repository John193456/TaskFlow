...
try {
    ...
    if ($method === 'GET') {
        // Browser is asking for the latest saved dashboard data for this account only.
        echo json_encode(['ok' => true, 'state' => taskflow_load_state((int) $sessionUser['id'])]);
        exit;
    }
    ...
    if ($method === 'POST') {
        ...
        taskflow_save_state($payload['state'], (int) $sessionUser['id']);
