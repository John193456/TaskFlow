...
function stats() {
  ...
  const weekHours = totalWorkSeconds() / 3600;
  return { todayTasks, completedToday, remindersToday, handledReminders, weekHours };
}

function workSessionSeconds(session) {
  /*
   * New timer sessions store durationSeconds so short sessions stay accurate.
   * Older sessions only have minutes, so this fallback keeps old data working.
   */
...
function totalWorkSeconds() {
  return state.workSessions.reduce((sum, session) => sum + workSessionSeconds(session), 0);
}
...
function formatDuration(seconds) {
  ...
  if (remainingSeconds) parts.push(`${remainingSeconds} second${remainingSeconds === 1 ? "" : "s"}`);
  return parts.join(" ");
}
...
function buildSearchResults(term) {
  ...
  const work = state.workSessions.map(session => ({
    type: "Work",
    title: `${formatDuration(workSessionSeconds(session))} work session`,
...
function renderWork() {
  const total = totalWorkSeconds();
  document.getElementById("pageContent").innerHTML = `
    <section class="stats-grid">
      <article class="data-card cream"><small>TOTAL HOURS</small><strong>${(total / 3600).toFixed(1)}h</strong><p>all logged sessions</p></article>
...
function workSessionItems() {
  ...
  return state.workSessions.slice().reverse().map(session => `
    <article class="item">
      ...
      <div class="item-main"><span class="item-title">${formatDuration(workSessionSeconds(session))}</span><div class="item-meta">${session.date} - ${escapeHtml(session.note || "Focused work")}</div></div>
...
function averageDailyHours() {
  ...
  return (totalWorkSeconds() / 3600 / days).toFixed(1);
}
...
function stopWorkTimer() {
  ...
  const durationSeconds = Math.max(1, Math.round(workTimer.elapsed));
  state.workSessions.push({
    id: uid(),
    date: todayISO(),
    durationSeconds,
    minutes: Math.round((durationSeconds / 60) * 100) / 100,
    note: "Timer session"
  });
...
function manualWorkForm() {
  ...
  `, "Save session", () => {
    const minutes = Number(formValue("minutes")) || 0;
    state.workSessions.push({ id: uid(), date: formValue("date"), minutes, durationSeconds: Math.round(minutes * 60), note: formValue("note") });
    saveState();
    closeModal();
    renderWork();
  });
