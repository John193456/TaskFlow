...
try {
    ...
    if ($action === 'delete_user') {
        ...
        $target = taskflow_admin_fetch_user($pdo, $userId);
        if ($target && $target['role'] !== 'admin') {
            taskflow_delete_user_data($userId);
    ...
    if ($action === 'reset') {
        ...
        taskflow_log_admin_action($adminId, 'reset_demo_data', 'Admin restored own demo TaskFlow notebook.');
        $notice = 'Your admin TaskFlow notebook has been restored.';
    }
  ...
  <style>
    ...
    @media (max-width: 760px) {
      .admin-wrap {
        width: 100%;
        max-width: 560px;
        padding: max(12px, env(safe-area-inset-top)) clamp(12px, 3.6vw, 16px) 30px;
      }
      ...
      .admin-wrap .hero-card h2 { font-size: clamp(28px, 8vw, 38px); line-height: 1.05; }
      .admin-wrap .hero-card p { max-width: 28rem; font-size: 13px; }
      .admin-wrap .panel-card { margin-bottom: 14px; padding: 16px; border-radius: 22px; }
      .admin-controls-head { flex-direction: column; gap: 14px; }
      ...
      .admin-control-menu { grid-column: 1 / -1; width: 100%; }
      .admin-control-panel { width: 100%; }
      .admin-table { min-width: 860px; font-size: 12px; }
      .admin-table th, .admin-table td { padding: 8px; }
      .analytics-slider { grid-auto-columns: minmax(220px, 88%); gap: 10px; }
      .analytics-slide { min-height: 180px; padding: 14px; }
      .admin-chart-grid { gap: 12px; margin-bottom: 14px; }
      .admin-chart-card { min-height: 240px; }
      .admin-chart-card canvas { max-height: 180px; }
      .admin-stats-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
      .admin-stats-grid .data-card {
        min-height: 96px;
        padding: 12px;
        border-radius: 18px;
      }
      .admin-stats-grid .data-card small {
        min-height: 0;
        margin-bottom: 8px;
        font-size: 9px;
        line-height: 1.15;
      }
      .admin-stats-grid .data-card strong { font-size: clamp(24px, 8vw, 32px); }
      .admin-stats-grid .data-card p { margin-top: 4px; font-size: 12px; }
    ...
    @media (max-width: 520px) {
      .admin-wrap { padding: 10px 10px 26px; }
      .admin-wrap .hero-card { min-height: 118px; padding: 16px; }
      .admin-wrap .hero-card h2 { font-size: clamp(24px, 7.5vw, 32px); }
      .admin-wrap .panel-card { padding: 14px; }
      .admin-scroll { margin: 0 -8px; padding: 0 8px; }
      .admin-filters { grid-template-columns: 1fr; }
      .admin-stats-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; }
      .admin-stats-grid .data-card { min-height: 84px; padding: 10px; border-radius: 16px; }
      .admin-stats-grid .data-card small { font-size: 8px; margin-bottom: 6px; }
      .admin-stats-grid .data-card strong { font-size: 24px; }
      .admin-stats-grid .data-card p { font-size: 11px; }
      .admin-control-actions { grid-template-columns: repeat(3, minmax(0, 1fr)); }
      .admin-control-actions > button { padding: 0 6px; font-size: 11px; }
    ...
    @media (max-width: 360px) {
      ...
      .admin-control-actions { grid-template-columns: 1fr; }
    }
  </style>
</head>
<body>
